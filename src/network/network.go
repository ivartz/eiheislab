...
func TCPServerInit(localListenPort int, send_ch, receive_ch chan Tcp_message) error{
	//var buffer [1024]byte
 	...
 	defer func() {
		if r := recover(); r != nil {
			fmt.Printf("TCPServerInit: ERROR recovering in tcp_init: %s \n", r)
		}
	}()
	...
	if err != nil {
		fmt.Println("TCPServerInit: Could not resolve baddr")
		return err
	}

	tempConn, err := net.DialUDP("udp4", nil, baddr)
	if err != nil {
		fmt.Println("TCPServerInit: Failed to dial baddr for tempLAddr generation")
		return err
	}

	fmt.Printf("TCPServerInit: UDP-dialed %v to resolve its local address\n", baddr) // midlertidig
	//fmt.Println(baddr)
	...
	// Using tempLAddr to make tcp connection to listen to
 	...
 	if err != nil{
 		fmt.Println("TCPServerInit: Failed to ResolveTCPAddr")
 		return err
 	}
	...
	if err != nil {
		fmt.Println("TCPServerInit: Failed to initialize ListenTCP")
		return err
	}
	fmt.Printf("TCPServerInit: Now listening on %v\n", laddr)
...
func tcp_transmit_server (s_ch chan Tcp_message){
	for {
		msg := <- s_ch
		fmt.Println("tcp_transmit_server: New message to send")
		...
		if (ok != true) {
			...
			err := errors.New("tcp_transmit_server: Failed to add newConn to conn_list map")
			panic(err)
		} else {
			n, err := sendConn.Write([]byte(msg.Data))	
			conn_list_mutex.Unlock()
			if err != nil || n < 0 {
				fmt.Printf("tcp_transmit_server: Write error (deleting remote address): %s\n",err)
				conn_list_mutex.Lock()
				sendConn.Close()
...
func tcp_handle_server (listener *net.TCPListener, r_ch chan Tcp_message){
	for {
		...
		fmt.Println("tcp_handle_server: Received new request for connection")
		if err != nil {
			fmt.Printf("tcp_handle_server: Error accepting tcp conn \n")
			panic(err)
		}
		...
		go func (raddr string, conn *net.TCPConn, ch chan Tcp_message){ 
			fmt.Println("tcp_handle_server: Starting new tcp read server because a new connection was accepted")
			buf := make([]byte,1024)
			for {
					n, err :=	conn.Read(buf)
					if err != nil || n < 0 {
						fmt.Printf("tcp_handle_server: Read error: %s \n",err)
						conn_list_mutex.Lock()
						conn.Close()
...
func new_tcp_conn(raddr string) bool{
	fmt.Println("new_tcp_conn: Trying to establish new tcp connection")
	//create address
	addr, err := net.ResolveTCPAddr("tcp4", raddr)
	if err != nil {
		fmt.Println("new_tcp_conn: ERROR: could not resolve address")
		return false
	}
	for {
		...
		if err != nil {
			fmt.Printf("new_tcp_conn: DialTCP to %v failed \n", raddr)
